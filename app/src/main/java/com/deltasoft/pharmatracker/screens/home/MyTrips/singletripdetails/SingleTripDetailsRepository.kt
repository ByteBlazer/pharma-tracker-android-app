...
class SingleTripDetailsRepository(var viewModel: SingleTripDetailsViewModel) {
    ...
    fun getRecentSignature(token: String,tripId: String,docId: String) {
        viewModelScope.launch {
            try {
                val response = RetrofitClient.apiService.getRecentSignature(token = token, tripId = tripId, docId = docId)
                if (response.isSuccessful) {
                    viewModel.updateRecentSignatureState(response.code(), response.message(),response?.body())
                } else {
                    val errorBodyString = response.errorBody()?.string()
                    if (errorBodyString != null) {
                        try {
                            val errorResponse =
                                Gson().fromJson(errorBodyString, ApiResponse::class.java)
                            val errorMessage = errorResponse.message
                            viewModel.updateRecentSignatureState(response.code(), errorMessage ?: "")
                        } catch (e: Exception) {
                            // Catch JSON parsing errors if the error body format is unexpected
                            println("Failed to parse error body: ${e.message}")
                            viewModel.updateRecentSignatureState(0, "${e.message}")
                        }
                    }
                }
            } catch (e: Exception) {
                // Handle network errors
                println("Network error: ${e.message}")
                viewModel.updateRecentSignatureState(0, AppConstants.NETWORK_LOSS_MESSAGE)
            }
        }
    }
}